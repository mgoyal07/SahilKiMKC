...
document.addEventListener("DOMContentLoaded", () => {
    ...
    yesButton.addEventListener('click', () => {
        alert('Thank You sooooooooooo....... much!');
    });

    noButton.addEventListener('click', () => {
        noClicks++;
        if (noClicks < 5) {
            message.textContent = `Maanja na! please (${noClicks} clicks)`;
        }else if (noClicks == 7) {
            message.textContent = `Maanja na! please (THALA clicks)`;
        }
         else {
            message.textContent = `Puchta rahunga jab tk nhi maanegi! (${noClicks} PLEASE 😖🙏)`;
        }
    });
});
